...
public abstract class BaseAction extends MoskitoAction{
	 ...
	 */
	public ActionForward moskitoExecute(
		...
		try{
			try{
				init(mapping,bean,req,res);
			}catch(AbortActionExecutionException e){
				return null;
			}
			...
			log.error("BaseAction: unhandled Exception ",e);
			return handleError(req, res, e, mapping);
		}finally{
			deInit(mapping,bean,req,res);
		}
	}
	...
	public String getAppName(HttpServletRequest request){
		return getResources(request).getMessage("application.name");
	}
	...
	protected void addErrors(HttpServletRequest req,ActionErrors errors){
